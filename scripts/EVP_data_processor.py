...
def load_and_process_data():
    ...
    for program in programs:
        for variant in variants:
            filename = f"{program}-stats-{variant}.csv"
        ...
        if not vanilla_data.empty and not evp_data.empty:
            ...
            characteristics = {
                ...
                'SolverTime Reduction (%)': round(((vanilla_row['FinalSolverTime'] - evp_row['FinalSolverTime']) / 
                                                   vanilla_row['FinalSolverTime'] * 100) if vanilla_row['FinalSolverTime'] > 0 else 0, 1),
                'Query Reduction (%)': round(((vanilla_row['TotalQueries'] - evp_row['TotalQueries']) / 
                                             vanilla_row['TotalQueries'] * 100) if vanilla_row['TotalQueries'] > 0 else 0, 1),
...
def generate_latex_tables(rq1_df, rq2_df, rq3_df):
    ...
    latex_output.append("\\toprule")
    latex_output.append("\\textbf{Program} & \\textbf{Size} & \\textbf{Complexity} & \\textbf{QueryTime} & \\textbf{SolverTime} & \\textbf{Queries} & \\textbf{Coverage} & \\textbf{Memory} \\\\")
    ...
    for _, row in rq3_df.iterrows():
        latex_output.append(f"\\texttt{{{row['Program']}}} & {row['Instructions']:,} & {row['Complexity']} & "
                          f"{row['QueryTime Reduction (%)']:.1f} & {row['SolverTime Reduction (%)']:.1f} & "
                          f"{row['Query Reduction (%)']:.1f} & {row['Coverage Delta']:+d} & {row['Memory Savings (MB)']:.1f} \\\\")
...
def main():
    ...
    if df.empty:
        print("[!] No data files found. Please check that CSV files are in the current directory.")
        return
    ...
    print("-" * 40)
    avg_improvements = rq1_df[rq1_df['Metric'].str.contains('Time|Queries')].groupby('Metric')['Improvement (%)'].mean()
    ...
    for _, row in rq3_df.iterrows():
        print(f"  {row['Program']}: {row['Complexity']} complexity, "
              f"QueryTime reduction {row['QueryTime Reduction (%)']:.1f}%, "
